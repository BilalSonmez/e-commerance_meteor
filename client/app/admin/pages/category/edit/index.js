import { FlowRouter } from 'meteor/ostrio:flow-router-extra';


Template.adminPagesCategoryEdit.onCreated(function() {
    this.state = new ReactiveDict(null, {
        data: {},
        category: []
    });
});

Template.adminPagesCategoryEdit.onRendered(function() {
    const self = this;
    const _id = FlowRouter.getParam("_id");
    this.autorun(function () {
        AppUtil.refreshTokens.get('category');
        Meteor.call('category.show', {_id: _id}, function (error, result) {
            if (error) {
                ErrorHandler.show(error)
                return;
            }
            self.state.set('data', result);
        });

        Meteor.call('category.list', {}, function (error, result) {
            if (error) {
                ErrorHandler.show(error)
                return;
            }
            console.log(result.category);
            self.state.set('category', result.category);
        });
    });
});

Template.adminPagesCategoryEdit.events({ 
    'submit #categoryEdit': function(event, template) { 
        event.preventDefault();
        let title = event.target.title.value
        let category = event.target.subCategory.value == "null" ? null : event.target.subCategory.value; 
        const obj = {
            _id: FlowRouter.getParam("_id"),
            category: {
              title: title,
              subCategory: category
            }
        };

        Meteor.call('category.update', obj, function (error, result) {

            if (error) {
              console.log(error);
              return;
            }
      
            event.target.reset();
            document.location.reload(true);
        });
    } 
});